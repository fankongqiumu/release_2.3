...
public class SDWindowManager
{
    ...
    private ArrayList<WeakReference<View>> listView = new ArrayList<>();
    ...
    {
        WindowManager windowManager = (WindowManager) SDLibrary.getInstance().getApplication().getSystemService(Context.WINDOW_SERVICE);
    ...
    {
        Iterator<WeakReference<View>> it = listView.iterator();
        while (it.hasNext())
        {
    ...
    public List<View> getView(Class clazz)
    {
        ...
        if (clazz != null)
        {
            for (WeakReference<View> item : listView)
            {
                View itemView = item.get();
                if (itemView != null)
                {
