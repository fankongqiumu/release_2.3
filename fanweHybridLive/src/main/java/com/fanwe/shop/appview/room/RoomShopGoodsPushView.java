...
public class RoomShopGoodsPushView extends RoomView
{
    ...
    public RoomShopGoodsPushView(Context context, String createrId, int goods_id)
    {
        super(context);
        this.anchorId = createrId;
    ...
    public void bindData(CustomMsgShopPush customMsgShopPush)
    {
        ...
        if (customMsgShopPush != null && customMsgShopPush.getGoods() != null)
        {
            ...
            if (goods.getType() == 1)
            {
                SDViewUtil.hide(ll_give);
            } else
            {
                if (!isCreater())
                {
                    SDViewUtil.show(ll_give);
                } else
                {
                    SDViewUtil.hide(ll_give);
                }
    ...
    private boolean isCreater()
    {
        ...
        if (!anchorId.equals(userModel.getUser_id()))
        {
            return false;
        } else
        {
    ...
    {
        ShopCommonInterface.requestShopGoodsUrl(goods_id, Integer.parseInt(anchorId), new AppRequestCallback<App_pai_user_open_goods_urlActModel>()
        {
