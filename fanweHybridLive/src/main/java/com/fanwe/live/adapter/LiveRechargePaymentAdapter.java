...
public class LiveRechargePaymentAdapter extends SDSimpleAdapter<PayItemModel>
{
    ...
    private ItemClickListener<PayItemModel> clickPaymentListener;

    public void setClickPaymentListener(ItemClickListener<PayItemModel> clickPaymentListener)
    {
    ...
    public void bindData(final int position, View convertView, ViewGroup parent, final PayItemModel model)
    {
        ...
        if (model.isSelected())
        {
            iv_selected.setImageResource(R.drawable.ic_live_pop_choose_on);
        } else
        {
