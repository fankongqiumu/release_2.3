...
public class LiveCreateRoomActivity extends BaseActivity implements TextWatcher, TextView.OnEditorActionListener, TencentLocationListener
{
    ...
    private FrameLayout rl_room_image;
    ...
    private void setShareShow()
    {
        if (isSinaEnable())
        {
            SDViewUtil.show(iv_share_weibo);
        } else
        {
            SDViewUtil.hide(iv_share_weibo);
        }

        if (isQQEnable())
        {
            SDViewUtil.show(iv_share_qq);
            SDViewUtil.show(iv_share_qqzone);
        } else
        {
            SDViewUtil.hide(iv_share_qq);
            SDViewUtil.hide(iv_share_qqzone);
        }

        if (isWeixinEnable())
        {
            SDViewUtil.show(iv_share_timeline);
            SDViewUtil.show(iv_share_wechat);
        } else
        {
            SDViewUtil.hide(iv_share_timeline);
            SDViewUtil.hide(iv_share_wechat);
        }
    }
...
//        String province = "";
    ...
    public void onEventMainThread(EOnActivityResult event)
    {
        if (event.activity == this)
        {
            ...
            ImageCropManage.onActivityResult(this, event.requestCode, event.resultCode, event.data);
        }
    }
    ...
    private void requestCreatetLive()
    {
        ...
        CommonInterface.requestAddVideo(image_path, getTopic(), getCateId(), getCity(), getProvince(), getShareType(), isLocate, isPrivate, new AppRequestCallback<Video_add_videoActModel>()
        {
            @Override
