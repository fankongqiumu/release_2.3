    ...
    private List<UserModel> parseUserData(List<List<String>> listData)
    {
        ...
        try
        {
            for (List<String> item : listData)
            {
                ...
                if (!TextUtils.isEmpty(headImage) && !headImage.startsWith(LiveConstant.DEFAULT_IMAGE_SCHEME))
                {
                    headImage = short_url + headImage;
                }
                if (!TextUtils.isEmpty(vIcon) && !vIcon.startsWith(LiveConstant.DEFAULT_IMAGE_SCHEME))
                {
