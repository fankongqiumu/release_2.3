...
public class IMHelper
{
    ...
    public static void loginIM(String userId, String userSig)
    {
        ...
        int sdkAppId = LiveConstant.APP_ID_TENCENT_LIVE;

        TIMUser user = new TIMUser();
        user.setAppIdAt3rd(String.valueOf(sdkAppId));
        user.setAccountType(LiveConstant.ACCOUNT_TYPE_TENCENT_LIVE);
        ...
        TIMManager.getInstance().login(sdkAppId, user, userSig, new TIMCallBack()
        {
            ...
            {
                LogUtil.e("login error:" + errCode + "," + errMsg);
                ReporterUtil.reportUserError("login error:" + errCode + "," + errMsg);
                isInLogin = false;
