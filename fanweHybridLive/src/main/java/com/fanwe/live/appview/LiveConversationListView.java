...
public class LiveConversationListView extends BaseAppView
{
    ...
    private void bindAdapter()
    {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            ...
            {
                LiveConversationListModel model = adapter.getItem((int) id);

                if (onItemClickListener != null)
                {
                    IMHelper.setSingleC2CReadMessage(model.getPeer(), false);
                    model.updateUnreadNumber();
                    adapter.updateItem(model);
                    notifyTotalUnreadNumListener();
                    onItemClickListener.onItemClickListener(model);
                }
            }
        });
