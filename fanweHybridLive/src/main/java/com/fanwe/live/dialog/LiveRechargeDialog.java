...
//    private boolean openExchange = true;
    ...
    private void initTabs() {
        if(!AppRuntimeWorker.useGameCoin()) {
            SDViewUtil.hide(tab_game_exchange);
        }
        ...
        tab_recharge.getViewConfig(tab_recharge.mTvTitle).setTextColorNormal(Color.parseColor("#80FFFFFF")).setTextColorSelected(SDResourcesUtil.getColor(R.color.white));
    ...
    private void requestData() {
        ...
        SDViewUtil.show(progress);
        SDViewUtil.invisible(tv_empty_text);
        ...
        CommonInterface.requestPay(pay_id, rule_id, money, new AppRequestCallback<App_payActModel>()
        {
            ...
            private void openSDKPAY(PaySdkModel model)
            {
                ...
                if (!TextUtils.isEmpty(payCode))
                {
                    ...
                    } else if(PaymentType.WFTWXPAY.equalsIgnoreCase(payCode)) {
                        CommonOpenSDK.WFTWxAppPay(model.getWFTWxAppPayModel(), getOwnerActivity());
                    } else if(PaymentType.YJWAP.equalsIgnoreCase(payCode)) {
                        openPayWap(model.getYJWAPPayModel());
                    }
                } else
                {
