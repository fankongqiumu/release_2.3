...
public class LiveGiftPlayView extends LinearLayout implements ILiveGiftView<ILiveGiftMsg>
{
    ...
    {
        LiveUserInfoDialog dialog = new LiveUserInfoDialog((Activity) getContext(), msg.getMsgSender().getUser_id());
        dialog.show();
    }
    ...
    {
        delayRunnable.runDelay(DURATION_DELAY);
        isPlayingDelay = true;
    }

    private void removeDelayRunnable()
    {
        delayRunnable.removeDelay();
        isPlayingDelay = false;
    }

    private SDDelayRunnable delayRunnable = new SDDelayRunnable()
    {
