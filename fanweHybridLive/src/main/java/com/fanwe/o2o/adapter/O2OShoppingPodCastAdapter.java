...
public class O2OShoppingPodCastAdapter extends SDSimpleAdapter<App_shop_mystoreItemModel>
{
    ...
    private ItemClickListener<App_shop_mystoreItemModel> clickToDetailListener;
    private ItemClickListener<App_shop_mystoreItemModel> clickPushListener;
    private UserModel dao = UserModelDao.query();

    public void setClickToDetailListener(ItemClickListener<App_shop_mystoreItemModel> clickToDetailListener)
    {
        this.clickToDetailListener = clickToDetailListener;
    }

    public void setClickPushListener(ItemClickListener<App_shop_mystoreItemModel> clickPushListener)
    {
    ...
    public void bindData(final int position, View convertView, ViewGroup parent, final App_shop_mystoreItemModel model)
    {
        ...
        if (dao.getUser_id().equals(createrId))
        {
            if (isPlayback)
            {
                SDViewUtil.hide(txv_add_cart);
            } else
            {
                SDViewUtil.show(txv_add_cart);
            }
