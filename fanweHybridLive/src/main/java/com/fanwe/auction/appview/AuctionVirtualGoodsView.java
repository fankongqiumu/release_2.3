...
//        dateType = mAdapterLabel.getSelectManager().getSelectedItem().getName();
        mAdapterLabel.setItemClickListener(new SDAdapter.ItemClickListener<GoodsTagsModel>()
        {
            @Override
            public void onClick(int position, GoodsTagsModel item, View view)
            {
    ...
    {
        mPickerDialog = new LiveTimePickerDialog.Builder()
                .setCallBack(this)
                ...
                .setWheelItemTextSelectorColor(getResources().getColor(R.color.text_picker_selected))
                .build();
    }
    ...
    public void onEventMainThread(EOnActivityResult event)
    {
        ...
        if (event.activity == getActivity())
        {
            ...
            ImageCropManage.onActivityResult(getActivity(), event.requestCode, event.resultCode, event.data);
        }
    }
